...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:wialon.e25838e5c59d.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
      ...
      __eY : function(bv, bD, bu){
        ...
        if(bt == wialon.item.MUnitSensor.invalidValue){
          ...
          if(bv == f){
            ...
            if(!bD.pos)return wialon.item.MUnitSensor.invalidValue;
            bt = bD.pos.x;
          } else if(/^in\d\d?$/.test(bv)){

            if(!(bD.f & 0x2))return wialon.item.MUnitSensor.invalidValue;
            var br = parseInt(bv.substr(2));
            if(br < 1 || br > 32 || isNaN(br))return this.__fa(bu, bD.i);
            var bB = 1 << (br - 1);
            bt = (bD.i & bB) ? 1 : 0;
          } else if(/^out\d\d?$/.test(bv)){

            if(!(bD.f & 0x4))return wialon.item.MUnitSensor.invalidValue;
            var br = parseInt(bv.substr(3));
            if(br < 1 || br > 32 || isNaN(br))return this.__fa(bu, bD.o);
            var bB = 1 << (br - 1);
            bt = (bD.o & bB) ? 1 : 0;
    ...
    members : {
      execReport : function(i, h, d, f, g){
        ...
        return wialon.core.Remote.getInstance().remoteCall(b, {
          ...
          reportObjectSecId : d,
          interval : f
        }, qx.lang.Function.bind(this.__ff, this, wialon.util.Helper.wrapCallback(g)), 180);
      },
      cleanupResult : function(j){

        return wialon.core.Remote.getInstance().remoteCall(a, {
        }, qx.lang.Function.bind(this.__fg, this, wialon.util.Helper.wrapCallback(j)));
      },
      __ff : function(l, m, k){
